...
function crearTarjetas(arrayDeProductos, contenedor, condicion) {
    ...
    arrayDeProductos.forEach(elemento => {
       ...
       producto.classList.add('estiloTarjetas')
        producto.innerHTML = `
        <h1>${elemento.nombre}</h1>
        <h3>$${elemento.precio}</h3>
        <h3> ${elemento.cantidad === undefined ? "" : "cantidad: " + elemento.cantidad}</h3>
        <img class= "img"  src="${elemento.img}">
        <button class="btn btn-dark" id = "${elemento.id}">${condicion ? "Agregar al carrito" : "Eliminar producto"} </button>
        `
...
function filtroBuscador(e) {
    let arrayBuscador = arrayDeProductos.filter(producto => producto.nombre.includes(e.target.value) )
...
function filtrarPorCategoria(categoria) {
    ...
    crearTarjetas(arrayFiltrado, contenedor, true)
    visibilizarTarjetas(true)
    visibilizarCarrito(false)
}
